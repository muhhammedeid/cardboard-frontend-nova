# Cardboard Management — Nova Frontend (الواجهة المعتمدة)
...
الـbackend ونفس مسارات الشاشات** دون أي تعديل على قواعد البيانات أو منطق الأعمال.

> **قرار المنتج:** هذه الواجهة هي **المعتمدة** بديلًا كاملًا عن `frontend/` القديمة (تخلٍّ كامل عنها).
...
- **المسودات لا تُحتسب في أي مجموع**: تُعلَّق التوريدة/المصروف/الدفعة كمسودة، ولا تدخل المخزون أو
  الحسابات إلا عند الاعتماد (وقد أُثبت ذلك بقياس فعلي)
- **تعديل فردي لكل صف**: زر تعديل في صف التوريدة/المورد/الدفعة/المصروف/البيع (للمسودات)
- **طباعة كارتة الوزن** بنفس شكل التذكرة الورقية التشغيلية (`/printview` من نفس الأصل)
