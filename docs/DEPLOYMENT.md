...
### 3.2 الدخول من داخل التطبيق

عند انتهاء الجلسة لم يعد المشغّل بحاجة لمغادرة Nova: البطاقة نفسها تحمل نموذج **البريد + كلمة المرور** يرسل إلى
`/api/method/login` على نفس الأصل، ثم يعيد قراءة الجلسة ويعود بك إلى نفس الشاشة التي كنت فيها.

- أي رفض من الخادم يظهر بنصه العربي في البطاقة («البريد الإلكتروني أو كلمة المرور غير صحيحة») بدل الرجوع الصامت.
- زر **«الدخول عبر صفحة النظام»** باقٍ لمن يستخدم OTP أو «الدخول برابط البريد».
- الدخول من داخل Nova يضبط **نفس كوكي الجلسة** الذي يستخدمه Desk (نفس المضيف)، فلا حاجة لتسجيل دخول ثانٍ.
- تم التحقق في Chrome حقيقي: بعد الدخول استدعت الواجهة `get_session_context` ثم `lookups` ثم `list_supplies`،
  أي أنها فتحت شاشة التوريدات فعلًا.
